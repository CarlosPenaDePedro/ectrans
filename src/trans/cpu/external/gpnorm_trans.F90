...
USE GPNORM_TRANS_CTL_MOD, ONLY : GPNORM_TRANS_CTL,GPNORM_TRANS_CTL_LGP
...
LOGICAL, OPTIONAL, INTENT(IN) :: USE_LGP
!ifndef INTERFACE
...
CALL SET_RESOL(KRESOL)
  ...
  IF (PRESENT(USE_LGP)) THEN
    IF (USE_LGP) THEN
      CALL GPNORM_TRANS_CTL_LGP(PGP, KFIELDS, KPROMA, PAVE, PMIN, PMAX, LDAVE_ONLY, F%RW)
    ELSE
