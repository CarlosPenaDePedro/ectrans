...
SUBROUTINE GPNORM_TRANS(PGP,KFIELDS,KPROMA,PAVE,PMIN,PMAX,LDAVE_ONLY, KRESOL, LREPRO)
...
!     LREPRO      - T full reproducible F reproducible with same subdomain but less communication (optional default T) 
...
USE GPNORM_TRANS_CTL_MOD, ONLY : GPNORM_TRANS_CTL, GPNORM_TRANS_CTL_MPIDEP
...
LOGICAL,OPTIONAL,  INTENT(IN)    :: LREPRO
!ifndef INTERFACE
...
IF (PRESENT(LREPRO)) THEN
  IF (.NOT. LREPRO) THEN
    ! Non-always-reproducible grid-point norm (results depend on MPI decomposition)
    CALL GPNORM_TRANS_CTL_MPIDEP(PGP, KFIELDS, KPROMA, PAVE, PMIN, PMAX, LDAVE_ONLY, F%RW)
  ELSE
    CALL GPNORM_TRANS_CTL          (PGP, KFIELDS, KPROMA, PAVE, PMIN, PMAX, LDAVE_ONLY, F%RW)
  END IF
ELSE
  CALL GPNORM_TRANS_CTL            (PGP, KFIELDS, KPROMA, PAVE, PMIN, PMAX, LDAVE_ONLY, F%RW)
END IF
