...
SUBROUTINE GPNORM_TRANS_CTL_MPIDEP(PGP,KFIELDS,KPROMA,PAVE,PMIN,PMAX,LDAVE_ONLY,PW)


!**** *GPNORM_TRANS_CTL_MPIDEP* - calculate grid-point norms

!     Purpose.
!     --------
!     Grid-point norm using standard floating-point reductions.
!     Results are reproducible for a fixed MPI configuration, but vary when
!     the number of MPI tasks or the domain decomposition changes.
!
!     Selected when LREPRO = .FALSE.

!**   Interface.
!     ----------
!     CALL GPNORM_TRANS_CTL_MPIDEP(...)
...
!     Author.
!     -------
!        Based on original GPNORM_TRANS_CTL (G. Mozdzynski *ECMWF*)
!        refactored by C. Peña

!     Modifications.
!     --------------
!        21-Nov-2025  C. Peña
!                     Created MPIDEP variant using Kahan summation and
!                     MPI_Allreduce instead of Transposing to latitudes 
!                     doing a 2-stage communication pattern. 
!                     .
!
!     ------------------------------------------------------------------
  ...
  USE TPM_DISTR        , ONLY : D, NPROC,MYPROC  ! GP decomposition 
  USE MPL_MODULE       , ONLY : MPL_ALLREDUCE, MPL_SEND,MPL_RECV,MPL_BROADCAST,JP_BLOCKING_STANDARD
  USE ABORT_TRANS_MOD  , ONLY : ABORT_TRANS
  USE YOMHOOK          , ONLY : LHOOK, DR_HOOK, JPHOOK
  ...
  REAL(KIND=JPRD)                :: W, ZT, ZY
  REAL(KIND=JPRD), ALLOCATABLE   :: ZSUM(:), ZCOMP(:)   ! weighted sums (global-mean contributions)
  REAL(KIND=JPRB), ALLOCATABLE   :: ZMING(:), ZMAXG(:)
  REAL(KIND=JPRD), ALLOCATABLE   :: WLAT(:)
  REAL(KIND=JPRD), ALLOCATABLE :: ZBUF(:)   ! buffer for received partial sums
  INTEGER(KIND=JPIM)          :: STEP       ! tree stride: 1,2,4,8,...
  INTEGER(KIND=JPIM)          :: SRC, DST   ! partner ranks (1-based)
  INTEGER(KIND=JPIM)          :: ITAG_SUM   ! message tag
  ...
  ALLOCATE(ZCOMP(KFIELDS));  ZCOMP(:) = 0.0_JPRD
  ...
  DO JBLK = 1, NGPBLKS
    ...
    DO JI = 1, ICEND
      ...
      DO JF = 1, KFIELDS
        ZY = W * PGP(JI,JF,JBLK) - ZCOMP(JF)
        ZT = ZSUM(JF) + ZY
        ZCOMP(JF) = (ZT - ZSUM(JF)) - ZY
        ZSUM(JF)  = ZT
        IF (.NOT. LDAVE_ONLY) THEN
  ...
  IF (NPROC > 1) THEN
    ...
    ! Upward binary tree reduction on ZSUM
    ITAG_SUM = 9001_JPIM
    ALLOCATE(ZBUF(KFIELDS))

    STEP = 1_JPIM
    DO WHILE (STEP < NPROC)
      ! MYPROC is 1-based; use (MYPROC-1) in MOD to get 0-based pattern
      IF (MOD(MYPROC-1_JPIM, 2*STEP) == 0) THEN
        SRC = MYPROC + STEP
        IF (SRC <= NPROC) THEN
          CALL MPL_RECV(ZBUF, KSOURCE=SRC, KTAG=ITAG_SUM, &
                        KMP_TYPE=JP_BLOCKING_STANDARD)
          ZSUM = ZSUM + ZBUF
        END IF
      ELSE
        DST = MYPROC - STEP
        CALL MPL_SEND(ZSUM, KDEST=DST, KTAG=ITAG_SUM, &
                      KMP_TYPE=JP_BLOCKING_STANDARD)
        EXIT
      END IF
      STEP = STEP * 2_JPIM
    ...
    ! Broadcast final sum from root MYPROC=1 to everyone
    CALL MPL_BROADCAST(ZSUM, ITAG_SUM, 1_JPIM, CDSTRING='GPNORM_TRANS_CTL_MPIDEP:SUM')
   
 ! CALL MPL_ALLREDUCE(ZSUM, 'SUM', .FALSE.)                    
    IF (.NOT. LDAVE_ONLY) THEN
      CALL MPL_ALLREDUCE(ZMING, 'MIN')
      CALL MPL_ALLREDUCE(ZMAXG, 'MAX')
    END IF
  END IF
  ...
  END SUBROUTINE GPNORM_TRANS_CTL_MPIDEP
