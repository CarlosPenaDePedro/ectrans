...
IF (KIFC > 0 .AND. KDGLU > 0 ) THEN
  ...
  IF (ILA <= ITHRESHOLD .OR. .NOT. S%LUSEFLT) THEN
    ...
    IF (LLDOUBLE) THEN
      ZB = PACK_FOR_GEMM(ISKIP, KFC, KIFC, KDGLU, KSL, PAIA, PW)
  ...
  IF (ILS <= ITHRESHOLD .OR. .NOT. S%LUSEFLT) THEN
    ...
    IF (LLDOUBLE) THEN
      ZB = PACK_FOR_GEMM(ISKIP, KFC, KIFC, KDGLU, KSL, PSIA, PW)
      ...
      ELSE
        BLOCK
          ...
          DO JK=1,KFC,ISKIP
            IFLD=IFLD+1
            DO J=1,KDGLU
              ZB_D(J,IFLD)=REAL(PSIA(JK,KSL+J-1),JPRD)*PW(KSL+J-1)
            ENDDO
          ENDDO
...
FUNCTION PACK_FOR_GEMM(KSKIP, KFC, KIFC, KDGLU, KSL, PIN, PW) RESULT(POUT)
...
REAL(KIND=JPRB) :: POUT(KDGLU, KIFC)
...
DO JK = 1, KFC, KSKIP
  IFLD = IFLD + 1
  DO J = 1, KDGLU
    POUT(J, IFLD) = PIN(JK, KSL + J - 1) * REAL(PW(KSL + J - 1), JPRB)
  ENDDO
ENDDO
