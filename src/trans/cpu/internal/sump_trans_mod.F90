...
INTEGER(KIND=JPIM) :: JLOC,IGLAT,IBSET,NPTS 
...
! >>> Build local map: local packed GP index (1..D%NGPTOT) -> global latitude index (1..R%NDGL)
IBSET = MY_REGION_EW

IF (.NOT. ALLOCATED(D%NIGLLS)) ALLOCATE(D%NIGLLS(D%NGPTOT))
IPOS = 1

DO JLOC = 1, D%NLSTLAT(MY_REGION_NS) - D%NFRSTLAT(MY_REGION_NS) + 1
  NPTS = D%NONL(D%NPTRFLOFF + JLOC, IBSET)
  IF (NPTS > 0) THEN
    IGLAT = D%NFRSTLOFF + JLOC
    D%NIGLLS(IPOS:IPOS+NPTS-1) = IGLAT
    IPOS = IPOS + NPTS
  END IF
END DO
